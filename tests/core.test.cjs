const {secureStorage,syntheticNetwork}=require('./fixtures.cjs');
...
function config(t){const dir=fs.mkdtempSync(path.join(os.tmpdir(),'common-test-'));t.after(()=>fs.rmSync(dir,{recursive:true,force:true}));return {settings:new Settings(dir,{secureStorage}),dir,identity:memoryIdentity()};}
...
async function registrar(t,requests){return server(t,async(q,r)=>{let body='';for await(const c of q)body+=c;requests.push({method:q.method,url:q.url,headers:q.headers,body:body?JSON.parse(body):null});r.end(q.method==='POST'?'{"id":"fixture-id","node_token":"owner-token"}':q.url==='/health'?'{"status":"ok"}':'{}');});}
